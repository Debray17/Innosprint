...
const RoomAmenitiesPage = () => {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        const [amenitiesRes, roomsRes] = await Promise.all([
  ...
  const handleAddAmenity = async () => {
    ...
    try {
      ...
      const response = await createRoomAmenity(payload, { language: "en" });
      const newAmenity = {
  ...
  const handleEditAmenity = async () => {
    ...
    try {
      ...
      const response = await updateRoomAmenity(payload, { language: "en" });
      setAmenities((prev) =>
      prev.map((a) =>
  ...
  const handleDeleteAmenity = async () => {
    ...
    try {
      ...
      await deleteRoomAmenity(payload, { language: "en" });
  ...
  const resetForm = () => {
    setFormData({ roomId: "", name: "", description: "", icon: "" });
    setSelectedAmenity(null);
  };

  const getRoomLabel = (roomId) =>
  rooms.find((r) => r.id === roomId)?.roomNo || "Unknown Room";
  ...
  return (
    <Box>
        ...
        fullWidth>
            ...
            disabled={!formData.roomId || !formData.name || saving}>
