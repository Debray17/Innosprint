...
import { getPropertyAmenityList } from "../../services/propertyAmenityService";
import { getRoomList } from "../../services/roomService";
import { getRoomAmenityList } from "../../services/roomAmenityService";
...
export default function PropertyDetailsPage() {
  ...
  const amenityIconByKey = useMemo(
    () => ({
      wifi: WifiIcon,
      pool: PoolIcon,
      spa: SpaIcon,
    ...
    const buildViewModel = async () => {
      ...
      try {
        const [
          ...
          getPropertyById(id, { language: "en" }),
        ...
        const propertyAmenityList = Array.isArray(propertyAmenityResponse)
          ? propertyAmenityResponse
          : propertyAmenityResponse
            ? [propertyAmenityResponse]
            : [];
        ...
        const roomAmenityList = Array.isArray(roomAmenityResponse)
          ? roomAmenityResponse
          : roomAmenityResponse
            ? [roomAmenityResponse]
            : [];

        const roomTypesList = Array.isArray(roomTypesResponse)
          ? roomTypesResponse
          : roomTypesResponse
            ? [roomTypesResponse]
            : [];
        ...
        const amenitiesForProperty = propertyAmenityList
          .filter(
            (a) =>
              a?.isActive !== false &&
              a?.propertyId &&
              a.propertyId === propertyDto?.id,
          )
          .map((amenity) => {
            const iconKey = (amenity?.icon || "").toString().toLowerCase();
            return {
              name: amenity?.name || iconKey || "Amenity",
              icon: iconKey,
            };
          });

        const roomAmenitiesByRoomId = new Map();
        roomAmenityList
          .filter((a) => a?.isActive !== false)
          .forEach((amenity) => {
            const roomId = amenity?.roomId;
            if (!roomId) return;
            const prev = roomAmenitiesByRoomId.get(roomId) || [];
            prev.push({
              id: amenity?.id || amenity?.primaryKeyValue || `${roomId}:${prev.length}`,
              name: amenity?.name || "Amenity",
              description: amenity?.description || "",
              icon: (amenity?.icon || "").toString().toLowerCase(),
            });
            roomAmenitiesByRoomId.set(roomId, prev);
          });

        const roomsForProperty = roomsList
          .filter(
            (room) =>
              room?.isActive !== false &&
              room?.propertyId &&
              room.propertyId === propertyDto?.id,
          )
          .map((room, index) => {
            const roomType = roomTypeById.get(room?.roomTypeId) || null;
            const roomTypeName = roomType?.name || "Room";
            const roomKey = room?.id || room?.primaryKeyValue;
            const amenities = roomAmenitiesByRoomId.get(roomKey) || [];
            const basePrice = Number(room?.basePrise) || 0;
            return {
              id: roomKey || index,
              name: room?.roomNo ? `Room ${room.roomNo}` : roomTypeName,
              type: roomTypeName,
              description: room?.description || "",
              size: 0,
              maxGuests: Number(room?.capacity) || 1,
              bedType: roomTypeName,
              images: [
        ...
        const typeDto = propertyTypeById.get(propertyDto?.propertyTypeId) || null;
        const propertyTypeName =
          typeDto?.name || propertyDto?.icon || "Property";

        const minRoomPrice = roomsForProperty.length
          ? Math.min(...roomsForProperty.map((r) => Number(r.pricePerNight) || 0))
          : 0;

        const vm = {
          id: propertyDto?.id || id,
          type: propertyTypeName,
          stars: null,
          name: propertyDto?.name || "Property",
          location: {
            address: propertyDto?.address || "",
            city: propertyDto?.city || "",
            state: propertyDto?.country || "",
          },
          images: defaultImages,
          rating: Number(propertyDto?.rating) || 0,
          reviewsCount: Number(propertyDto?.reviewCount) || 0,
          description: propertyDto?.description || "",
          amenities: amenitiesForProperty,
          rooms: roomsForProperty,
          reviews: [],
          policies: {
            checkIn: propertyDto?.checkinDate
              ? new Date(propertyDto.checkinDate).toLocaleTimeString("en-US", {
                  hour: "numeric",
                  minute: "2-digit",
                })
              : "N/A",
            checkOut: propertyDto?.checkoutDate
              ? new Date(propertyDto.checkoutDate).toLocaleTimeString("en-US", {
                  hour: "numeric",
                  minute: "2-digit",
                })
              : "N/A",
          ...
          meta: {
            ...
            phoneNo: propertyDto?.phoneNo || "",
            commissionRate: propertyDto?.commissionRate ?? null,
            statusId: propertyDto?.statusId ?? null,
            minRoomPrice,
          },
        };

        if (isMounted) setPropertyVm(vm);
      } catch (err) {
        if (isMounted) {
          setLoadError(err?.message || "Failed to load property details.");
          setPropertyVm(propertyDetails);
        }
      } finally {
              ...
              <Box sx={{ p: 3 }}>
                ...
                <Box>
                    ...
                    <Grid container spacing={2}>
                      ...
                      const iconKey = (amenity?.icon || amenity?.name || "").
                      toString().
                      toLowerCase();
                      const IconComponent =
            ...
            <Paper sx={{ p: 3, position: "sticky", top: 100 }}>
              ...
              <Box
                ...
                <Typography variant="h4" fontWeight={700} color="primary">
                  Nu {(property.meta?.minRoomPrice ?? property.rooms[0]?.pricePerNight) || 0}
                </Typography>
                <Typography variant="body2" color="text.secondary">
                  / night
                </Typography>
